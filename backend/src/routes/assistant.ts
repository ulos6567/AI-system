...
router.post('/conversations/:id/messages', requireAuth, requireStoreScope(), async (req, res) => {
  const storeId = Number(req.params.storeId);
  const id = Number(req.params.id);
  const user = currentUser(req)!;
  ...
  const answer = await ask(storeId, user.id, parsed.data.message, id);
  res.json(answer);
});

router.post('/ask', requireAuth, requireStoreScope(), async (req, res) => {
  const storeId = Number(req.params.storeId);
  const user = currentUser(req)!;
  const parsed = AskSchema.safeParse(req.body ?? {});
  if (!parsed.success) {
  ...
  const answer = await ask(storeId, user.id, parsed.data.message);
  res.json(answer);
});
